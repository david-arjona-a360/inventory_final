...
def open_manage_users(session: dict, parent=None):
    ...
    def _add():
        ...
        tk.Button(add_win, text="Add User", width=16, command=_save_new).pack(pady=(16, 4))
        tk.Button(add_win, text="Cancel",   width=16, command=add_win.destroy).pack()
    ...
    def _change_role():
        ...
        tk.Button(role_win, text="Save", width=14, command=_save_role).pack(pady=6)
        tk.Button(role_win, text="Cancel", width=14, command=role_win.destroy).pack()

    tk.Button(btn_frame, text="➕  Add User",        width=16, command=_add).grid(       row=0, column=0, padx=6)
    tk.Button(btn_frame, text="🗑  Remove User",     width=16, command=_remove).grid(    row=0, column=1, padx=6)
    tk.Button(btn_frame, text="🔑  Change Password", width=16, command=_change_pwd).grid(row=0, column=2, padx=6)
    tk.Button(btn_frame, text="🔒  Change Role",     width=16, command=_change_role).grid(row=1, column=0, padx=6, pady=6, columnspan=3)
